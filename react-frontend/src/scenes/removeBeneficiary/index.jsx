...
//   const [selectedRowId, setSelectedRowId] = useState(null);
  ...
  const columns = [
    ...
    { field: 'beneficiaryNumber', headerName: 'BenNumber', cellClassName: 'pol-column--cell' },
    ...
    { field: 'customerId', headerName: 'Customer ID' },
    {
      ...
      renderCell: (params) => {
        const handleDelete = () => {
          const updatedBens = benList.beneficiaries.filter(
            (ben) => ben.id !== params.row.id
          );
          setBenList({ beneficiaries: updatedBens });
          const headers = new Headers();
          headers.append('Content-Type', 'application/json');
          headers.append('Authorization', 'Bearer ' + userToken);
          fetch(`http://localhost:8080/api/customer/${user.id}/beneficiary/${params.row.beneficiaryNumber}`, { method: 'DELETE', headers: headers })
...
export default RemoveBeneficiaries;